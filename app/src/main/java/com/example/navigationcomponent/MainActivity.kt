...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (navController != null) {
            ...
            val toggle = ActionBarDrawerToggle(
                this, drawerLayout, topAppBar,
                R.string.home, R.string.goto_first
            )
            drawerLayout.addDrawerListener(toggle)
            toggle.syncState()
            navController.addOnDestinationChangedListener { _, destination, _ ->
                topAppBar.title = destination.label ?: getString(R.string.app_name)
                if (destination.id == R.id.firstFragment) {
                    topAppBar.navigationIcon = getDrawable(R.drawable.ic_menu)
                    topAppBar.setNavigationOnClickListener {
                        drawerLayout.open() // Open drawer on menu icon click
                    }
                } else {
                    topAppBar.navigationIcon = getDrawable(R.drawable.ic_arrow_back)
                    topAppBar.setNavigationOnClickListener { navController.navigateUp() }
                }
            }
        } else {
            Log.e("MainActivity", "NavController is null! Check nav_host_fragment")
